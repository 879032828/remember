...
public class Activity_Registration extends BaseActivity implements OnClickListener {
	...
	public void onClick(View v) {
		...
		case R.id.bt_register_ok:
			...
			if (persondbdao.find(name)) {
				Toast.makeText(Activity_Registration.this, "该用户名已存在！", Toast.LENGTH_SHORT).show();
			} else {
				persondbdao.add(name, pwd2);
				Toast.makeText(getApplicationContext(), "注 册 成 功 ！", Toast.LENGTH_SHORT).show();
				Intent intent = new Intent(Activity_Registration.this, Activity_Login.class);
				startActivity(intent);
				finish();
			}
